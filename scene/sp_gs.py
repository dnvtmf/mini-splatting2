...
class SuperpointDeformationNetwork(nn.Module):
    def __init__(self, D=3, W=64, skips=(), input_ch=3, output_ch=6, multires=10, t_multires=6, dim_rot=4):
        super(SuperpointDeformationNetwork, self).__init__()
        self.D = D
        self.W = W
        ...
        self.gaussian_rotation = nn.Linear(W, dim_rot)
        # self.gaussian_scaling = nn.Linear(W, 3)

    def reset_parameters(self, zero_init):
        if zero_init:
            nn.init.constant_(self.gaussian_warp.weight, 0)
            if self.gaussian_warp.bias is not None:
                nn.init.constant_(self.gaussian_warp.bias, 0)
            nn.init.constant_(self.gaussian_rotation.weight, 0)
            if self.gaussian_rotation.bias is not None:
                nn.init.constant_(self.gaussian_rotation.bias, 0)
...
class DeformModel:
    ...
    def step(self, xyz, time_emb, use_mlp=True, not_sp=False):
        if not_sp:
            delta_xyz, delta_r = self.sp_deform(xyz, time_emb)
            return (delta_xyz, delta_r, 0), None
        if use_mlp:
